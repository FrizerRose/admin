...
export interface Actions {
  ...
  [LocalActionTypes.UPDATE_FAQ](
    { commit }: AugmentedActionContext & AugmentedSharedActionContext,
    payload: object
  ): Promise<Faq>;
  [LocalActionTypes.CREATE_FAQ](
    { commit }: AugmentedActionContext & AugmentedSharedActionContext,
    payload: object
  ): Promise<Faq>;
}
...
export const actions: ActionTree<State, RootState> & Actions = {
  ...
  async [LocalActionTypes.UPDATE_FAQ]({ commit }, faq: Faq): Promise<Faq> {
    return new Promise((resolve, reject) => (async () => {
      const response = await faqService.update(faq);
      if (response.status === 200 && response.data) {
        commit(LocalMutationTypes.CHANGE_FAQ_BEING_EDITED, response.data);
        resolve(response.data);
      } else {
        reject(new ApiError('Updating faq failed.'));
      }
    })());
  },
  async [LocalActionTypes.CREATE_FAQ]({ commit }, faq: Faq): Promise<Faq> {
    return new Promise((resolve, reject) => (async () => {
      const response = await faqService.create(faq);
